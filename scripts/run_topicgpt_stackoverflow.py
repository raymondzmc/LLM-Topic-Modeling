"""
Run TopicGPT pipeline on stackoverflow dataset.

Usage: python scripts/run_topicgpt_stackoverflow.py
"""
...
def load_config():
    with open("config_stackoverflow.yml", "r") as f:
...
def run_assignment(config):
    print("\n" + "=" * 60)
    print("STEP 3: Topic Assignment (eval sample)")
    print("=" * 60)
...
def run_evaluation(config):
    ...
    print("\n" + "=" * 60)
    print("RESULTS: TopicGPT on stackoverflow")
    print("=" * 60)
    ...
    for label in sorted(valid["label"].unique()):
        ...
        print(f"  Label {str(label):5s} -> {mapping}")
